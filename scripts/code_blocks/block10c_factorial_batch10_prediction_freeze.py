# ==========================================================================================
# COVA-3D — BLOCK 10C-FACTORIAL-BATCH-10-PRED-FREEZE
#
# BATCH
# -----
# fold3_seed17
...
BLOCK = (
    "10C-FACTORIAL-BATCH-10-PRED-FREEZE"
)

NEXT_BLOCK = (
    "10B-FACTORIAL-BATCH-11"
)
...
FIT_SUMMARY_PATH = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch10_fit_summary_v1_0.csv"
)
...
PRED_MANIFEST_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch10_prediction_freeze_v1_0.csv"
)
...
AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10c_factorial_batch10_prediction_freeze.json"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch10_prediction_freeze.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10c_factorial_batch10_prediction_freeze.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch10_predictions.sh"
    )
...
heading(
    "COVA-3D 10C — BATCH-10 IMAGE-ONLY PREDICTION FREEZE"
)
...
) != "10B-FACTORIAL-BATCH-10":

    raise RuntimeError(
        "Batch-10 fit is not the latest completed block."
    )


if state.get(
    "factorial_batch10_fit_status"
) != "COMPLETE":

    raise RuntimeError(
        "Batch-10 fit status is not COMPLETE."
    )
...
if len(
    ...
    raise RuntimeError(
        "Expected six Batch-10 fit rows."
    )
...
if set(
    ...
    raise RuntimeError(
        "Unexpected Batch-10 condition set."
    )
...
if len(
    ...
    raise RuntimeError(
        "Execution registry does not contain six Batch-10 runs."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify six Batch-10 execution rows."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify Batch-10 registry row."
    )
...
state[
    ...
    "FACTORIAL_BATCH10_PREDICTIONS_FROZEN_BATCH11_PENDING"
)
...
state[
    "factorial_batch10_prediction_status"
] = (
    "FROZEN"
)

state[
    "factorial_batch10_prediction_manifest"
] = str(
...
state[
    ...
    "Train the six frozen factorial conditions for fold3_seed29. "
...
project_state[
    ...
    "cova3d_factorial_batch11_pending"
)
...
print(
    "✓ Batch-10 execution status            : PREDICTIONS_FROZEN"
)

print(
    "✓ Prediction-frozen runs               : 30 /72"
)

print(
    "✓ Frozen probability maps              : 120"
)
...
def test_batch10_prediction_manifest():
...
def test_batch10_prediction_audit():
    ...
    assert audit["batch_id"] == "fold3_seed17"
...
def test_batch10_prediction_state():
    ...
    assert state["factorial_prediction_runs_frozen"] == 30
    assert state["factorial_prediction_maps_frozen"] == 120

    assert state["final_outer_cv_outcomes_authorized"] is False
    assert state["final_outer_cv_access_in_cova3d"] == 0

    assert state["next_factorial_batch"] == "fold3_seed29"
    assert state["next_block"] == "10B-FACTORIAL-BATCH-11"
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-10 prediction-freeze regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: freeze COVA-3D factorial batch 10 predictions [parallel]",
)
...
heading(
    "COVA-3D 10C — BATCH-10 PREDICTION FREEZE FINAL REPORT"
)
...
print(
    "BATCH-10 IMAGE-ONLY PREDICTIONS"
)
...
print(
    "Fits represented on branch             : 30 /72"
)

print(
    "Prediction-frozen runs on branch       : 30 /72"
)

print(
    "Frozen probability maps on branch      : 120"
)
...
print()
print(
    "STATUS                                 : BATCH10_PREDICTIONS_SAFELY_FROZEN"
)
...
print(
    "After I audit this result, Batch 07 will be backed up. Next scientific batch is fold3_seed29."
)
