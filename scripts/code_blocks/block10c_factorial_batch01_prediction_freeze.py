# ==========================================================================================
# COVA-3D — BLOCK 10C-FACTORIAL-BATCH-01-PRED-FREEZE
#
# BATCH
# -----
# fold0_seed17
...
BLOCK = (
    "10C-FACTORIAL-BATCH-01-PRED-FREEZE"
)
...
FIT_SUMMARY_PATH = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch01_fit_summary_v1_0.csv"
)
...
PRED_MANIFEST_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch01_prediction_freeze_v1_0.csv"
)
...
AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10c_factorial_batch01_prediction_freeze.json"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch01_prediction_freeze.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10c_factorial_batch01_prediction_freeze.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch01_predictions.sh"
    )
...
heading(
    "COVA-3D 10C — BATCH-01 IMAGE-ONLY PREDICTION FREEZE"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-01 fit is not the latest completed block."
    )


if state.get(
    "factorial_batch01_fit_status"
) != "COMPLETE":

    raise RuntimeError(
        "Batch-01 fit status is not COMPLETE."
    )
...
if len(
    ...
    raise RuntimeError(
        "Expected six Batch-01 fit rows."
    )
...
if set(
    ...
    raise RuntimeError(
        "Unexpected Batch-01 condition set."
    )
...
if len(
    ...
    raise RuntimeError(
        "Execution registry does not contain six Batch-01 runs."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify six Batch-01 execution rows."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify Batch-01 registry row."
    )
...
state[
    ...
    "FACTORIAL_BATCH01_PREDICTIONS_FROZEN_BATCH02_PENDING"
)
...
state[
    "factorial_batch01_prediction_status"
] = (
    "FROZEN"
)

state[
    "factorial_batch01_prediction_manifest"
] = str(
...
state[
    ...
    "Train the six frozen factorial conditions for fold0_seed29. "
...
project_state[
    ...
    "cova3d_factorial_batch02_pending"
)
...
print(
    "✓ Batch-01 execution status            : PREDICTIONS_FROZEN"
)
...
def test_batch01_prediction_manifest():
...
def test_batch01_prediction_audit():
    ...
    assert audit["batch_id"] == "fold0_seed17"
...
def test_batch01_prediction_state():
    ...
    assert state["factorial_runs_completed"] == 6
    assert state["factorial_prediction_runs_frozen"] == 6
    assert state["factorial_prediction_maps_frozen"] == 24

    assert state["final_outer_cv_outcomes_authorized"] is False
    assert state["final_outer_cv_access_in_cova3d"] == 0

    assert state["next_factorial_batch"] == "fold0_seed29"
    assert state["next_block"] == "10B-FACTORIAL-BATCH-02"
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-01 prediction-freeze regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: freeze COVA-3D factorial batch 01 predictions",
)
...
heading(
    "COVA-3D 10C — BATCH-01 PREDICTION FREEZE FINAL REPORT"
)
...
print(
    "BATCH-01 IMAGE-ONLY PREDICTIONS"
)
...
print(
    "Training batches completed             : 1 /12"
)
...
print(
    "After I audit this result, we move directly to Batch 02: fold0_seed29."
)
