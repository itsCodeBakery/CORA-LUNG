# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-10
#
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch10_epoch_log_v1_0.csv"
)
...
BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch10_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch10_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch10_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch10_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch10_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch10.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-10 — PREFLIGHT"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Unexpected frozen main-line state for parallel Batch-10 execution."
    )


if BATCH_ID != "fold3_seed17":

    raise RuntimeError(
        "Parallel Batch-10 authorization mismatch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-10 does not contain six runs."
    )
...
state[
    ...
    "FACTORIAL_BATCH10_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch10_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch10_seed"
] = (
    SEED
)

state[
    "factorial_batch10_models"
] = (
    6
)

state[
    "factorial_batch10_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch10_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "Batch-10 models before moving to fold3_seed29."
)
...
project_state[
    ...
    "cova3d_factorial_batch10_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-10-PRED-FREEZE."
)
...
def test_batch10_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-10"
    assert state["factorial_runs_completed"] == 30
    assert state["factorial_batches_completed"] == 5
    assert state["factorial_batch10_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch10_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-10 regression tests failed."
    )
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-10 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-10 — FINAL REPORT"
)
...
print(
    ...
    "for each model before this parallel Batch-10 run is backed up."
)
