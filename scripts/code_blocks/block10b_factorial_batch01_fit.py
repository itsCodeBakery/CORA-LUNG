# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-01
#
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch01_epoch_log_v1_0.csv"
)
...
BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch01_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch01_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch01_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch01_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch01_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch01.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-01 — PREFLIGHT"
)
...
if state.get(
    ...
    raise RuntimeError(
        "10A is not the current completed block."
    )
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-01 is not the next authorized batch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-01 does not contain six runs."
    )
...
ax.set_title(
    "COVA-3D Factorial Batch 01 — Fold 0, Seed 17"
)
...
state[
    ...
    "FACTORIAL_BATCH01_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch01_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch01_seed"
] = (
    SEED
)

state[
    "factorial_batch01_models"
] = (
    6
)

state[
    "factorial_batch01_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch01_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "Batch-01 models before moving to fold0_seed29."
)
...
project_state[
    ...
    "cova3d_factorial_batch01_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-01-PRED-FREEZE."
)
...
def test_batch01_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-01"
    assert state["factorial_runs_completed"] == 6
    assert state["factorial_batches_completed"] == 1
    assert state["factorial_batch01_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch01_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-01 regression tests failed."
    )
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-01 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-01 — FINAL REPORT"
)
...
print(
    "Factorial fits completed               : 6 /72"
)

print(
    "Paired fit batches completed           : 1 /12"
)
