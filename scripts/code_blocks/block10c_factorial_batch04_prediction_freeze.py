# ==========================================================================================
# COVA-3D — BLOCK 10C-FACTORIAL-BATCH-04-PRED-FREEZE
#
# BATCH
# -----
# fold0_seed29
...
PRED_MANIFEST_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch04_prediction_freeze_v1_0.csv"
)
...
AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10c_factorial_batch04_prediction_freeze.json"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch04_prediction_freeze.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10c_factorial_batch04_prediction_freeze.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch04_predictions.sh"
    )
...
heading(
    "COVA-3D 10C — BATCH-04 IMAGE-ONLY PREDICTION FREEZE"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-04 fit is not the latest completed block."
    )
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-04 fit status is not COMPLETE."
    )
...
if len(
    ...
    raise RuntimeError(
        "Expected six Batch-04 fit rows."
    )
...
if set(
    ...
    raise RuntimeError(
        "Unexpected Batch-04 condition set."
    )
...
if len(
    ...
    raise RuntimeError(
        "Execution registry does not contain six Batch-04 runs."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify six Batch-04 execution rows."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify Batch-04 registry row."
    )
...
state[
    ...
    "FACTORIAL_BATCH04_PREDICTIONS_FROZEN_BATCH05_PENDING"
)
...
state[
    "factorial_batch04_prediction_status"
] = (
    "FROZEN"
)

state[
    "factorial_batch04_prediction_manifest"
] = str(
...
state[
    ...
    "Train the six frozen factorial conditions for fold1_seed29. "
...
project_state[
    ...
    "cova3d_factorial_batch05_pending"
)
...
print(
    "✓ Batch-04 execution status            : PREDICTIONS_FROZEN"
)
...
def test_batch04_prediction_manifest():
...
def test_batch04_prediction_audit():
...
def test_batch04_prediction_state():
    ...
    assert state["next_factorial_batch"] == "fold1_seed29"
    assert state["next_block"] == "10B-FACTORIAL-BATCH-05"
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-04 prediction-freeze regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: freeze COVA-3D factorial batch 04 predictions [parallel]",
)
...
heading(
    "COVA-3D 10C — BATCH-04 PREDICTION FREEZE FINAL REPORT"
)
...
print(
    "BATCH-04 IMAGE-ONLY PREDICTIONS"
)
...
print(
    "Fits represented on branch             : 18 /72"
)

print(
    "Prediction-frozen runs on branch       : 18 /72"
)

print(
    "Frozen probability maps on branch      : 72"
)

print(
    "Fit batches represented on branch      : 3 /12"
)
...
print()
print(
    "STATUS                                 : BATCH04_PREDICTIONS_SAFELY_FROZEN"
)
...
print(
    "After I audit this result, Batch 04 will be backed up. Next scientific batch is fold1_seed29."
)
