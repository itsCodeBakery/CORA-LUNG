# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-05
#
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch05_epoch_log_v1_0.csv"
)
...
BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch05_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch05_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch05_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch05_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch05_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch05.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-05 — PREFLIGHT"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Unexpected frozen main-line state for parallel Batch-05 execution."
    )


# Parallel execution is allowed only for the already-frozen registry entry.
if BATCH_ID != "fold1_seed29":

    raise RuntimeError(
        "Parallel Batch-05 authorization mismatch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-05 does not contain six runs."
    )
...
state[
    ...
    "FACTORIAL_BATCH05_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch05_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch05_seed"
] = (
    SEED
)

state[
    "factorial_batch05_models"
] = (
    6
)

state[
    "factorial_batch05_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch05_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "Batch-05 models before moving to fold1_seed43."
)
...
project_state[
    ...
    "cova3d_factorial_batch05_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-05-PRED-FREEZE."
)
...
def test_batch05_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-05"
    assert state["factorial_runs_completed"] == 24
    assert state["factorial_batches_completed"] == 4
    assert state["factorial_batch05_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch05_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-05 regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: complete COVA-3D factorial batch 05 fits [parallel]",
)
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-05 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-05 — FINAL REPORT"
)
...
print(
    "Factorial fits represented on branch   : 24 /72"
)
...
print(
    ...
    "for each model before this parallel Batch-05 run is backed up."
)
