# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-06
#
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch06_epoch_log_v1_0.csv"
)
...
BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch06_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch06_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch06_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch06_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch06_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch06.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-06 — PREFLIGHT"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Unexpected frozen main-line state for parallel Batch-06 execution."
    )


if BATCH_ID != "fold1_seed43":

    raise RuntimeError(
        "Parallel Batch-06 authorization mismatch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-06 does not contain six runs."
    )
...
state[
    ...
    "FACTORIAL_BATCH06_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch06_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch06_seed"
] = (
    SEED
)

state[
    "factorial_batch06_models"
] = (
    6
)

state[
    "factorial_batch06_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch06_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "Batch-06 models before moving to fold2_seed17."
)
...
project_state[
    ...
    "cova3d_factorial_batch06_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-06-PRED-FREEZE."
)
...
def test_batch06_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-06"
    assert state["factorial_runs_completed"] == 24
    assert state["factorial_batches_completed"] == 4
    assert state["factorial_batch06_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch06_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-06 regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: complete COVA-3D factorial batch 06 fits [parallel]",
)
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-06 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-06 — FINAL REPORT"
)
...
print(
    ...
    "for each model before this parallel Batch-06 run is backed up."
)
