# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-04
#
...
NEXT_BLOCK = (
    "10C-FACTORIAL-BATCH-04-PRED-FREEZE"
)
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch04_epoch_log_v1_0.csv"
)

BATCH_RUN_SUMMARY_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch04_fit_summary_v1_0.csv"
)

BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch04_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch04_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch04_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch04_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch04_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch04.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-04 — PREFLIGHT"
)
...
) != "10C-FACTORIAL-BATCH-02-PRED-FREEZE":

    raise RuntimeError(
        "Batch-02 prediction freeze is not the frozen parallel-execution base."
    )
...
if state.get(
    ...
    raise RuntimeError(
        "Unexpected frozen main-line state for parallel Batch-04 execution."
    )


# PARALLEL EXECUTION RULE:
# Batch-03 is the next sequential main-line batch, but Batch-04 is
# independently frozen in the execution registry and may run on its
# dedicated branch without modifying main.
if BATCH_ID != "fold1_seed17":

    raise RuntimeError(
        "Parallel Batch-04 authorization mismatch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-04 does not contain six runs."
    )
...
state[
    ...
    "FACTORIAL_BATCH04_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch04_fit_status"
] = (
    "COMPLETE"
)

state[
    "factorial_batch04_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch04_seed"
] = (
    SEED
)

state[
    "factorial_batch04_models"
] = (
    6
)

state[
    "factorial_batch04_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch04_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "parallel Batch-04 models. Do not advance or overwrite main from this branch."
)
...
project_state[
    ...
    "cova3d_factorial_batch04_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-04-PRED-FREEZE."
)
...
def test_batch04_audit():
    ...
    assert audit["batch_id"] == "fold1_seed17"
...
def test_batch04_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-04"
    assert state["factorial_runs_completed"] == 18
    assert state["factorial_batches_completed"] == 3
    assert state["factorial_batch04_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch04_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-04 regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: complete COVA-3D factorial batch 04 fits [parallel]",
)
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-04 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-04 — FINAL REPORT"
)
...
print(
    "Parallel-branch fits represented       : 18 /72"
)

print(
    "Parallel-branch batches represented    : 3 /12"
)
...
print(
    ...
    "for each model before this parallel batch is integrated into main."
)
