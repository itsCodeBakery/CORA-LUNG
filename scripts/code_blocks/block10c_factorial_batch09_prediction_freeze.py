# ==========================================================================================
# COVA-3D — BLOCK 10C-FACTORIAL-BATCH-09-PRED-FREEZE
#
# BATCH
# -----
# fold2_seed43
...
PRED_MANIFEST_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch09_prediction_freeze_v1_0.csv"
)
...
AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10c_factorial_batch09_prediction_freeze.json"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch09_prediction_freeze.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10c_factorial_batch09_prediction_freeze.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch09_predictions.sh"
    )
...
heading(
    "COVA-3D 10C — BATCH-09 IMAGE-ONLY PREDICTION FREEZE"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-09 fit is not the latest completed block."
    )
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-09 fit status is not COMPLETE."
    )
...
if len(
    ...
    raise RuntimeError(
        "Expected six Batch-09 fit rows."
    )
...
if set(
    ...
    raise RuntimeError(
        "Unexpected Batch-09 condition set."
    )
...
if len(
    ...
    raise RuntimeError(
        "Execution registry does not contain six Batch-09 runs."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify six Batch-09 execution rows."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify Batch-09 registry row."
    )
...
state[
    ...
    "FACTORIAL_BATCH09_PREDICTIONS_FROZEN_BATCH10_PENDING"
)
...
state[
    "factorial_batch09_prediction_status"
] = (
    "FROZEN"
)

state[
    "factorial_batch09_prediction_manifest"
] = str(
...
state[
    ...
    "Train the six frozen factorial conditions for fold3_seed17. "
...
project_state[
    ...
    "cova3d_factorial_batch10_pending"
)
...
print(
    "✓ Batch-09 execution status            : PREDICTIONS_FROZEN"
)
...
def test_batch09_prediction_manifest():
...
def test_batch09_prediction_audit():
...
def test_batch09_prediction_state():
    ...
    assert state["next_factorial_batch"] == "fold3_seed17"
    assert state["next_block"] == "10B-FACTORIAL-BATCH-10"
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-09 prediction-freeze regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: freeze COVA-3D factorial batch 09 predictions [parallel]",
)
...
heading(
    "COVA-3D 10C — BATCH-09 PREDICTION FREEZE FINAL REPORT"
)
...
print(
    "BATCH-09 IMAGE-ONLY PREDICTIONS"
)
...
print()
print(
    "STATUS                                 : BATCH09_PREDICTIONS_SAFELY_FROZEN"
)
...
print(
    "After I audit this result, Batch 07 will be backed up. Next scientific batch is fold3_seed17."
)
