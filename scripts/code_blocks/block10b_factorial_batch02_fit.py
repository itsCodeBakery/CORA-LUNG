# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-02
#
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch02_epoch_log_v1_0.csv"
)
...
BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch02_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch02_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch02_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch02_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch02_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch02.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-02 — PREFLIGHT"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-01 prediction freeze is not the current completed block."
    )
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-02 is not the next authorized batch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-02 does not contain six runs."
    )
...
state[
    ...
    "FACTORIAL_BATCH02_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch02_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch02_seed"
] = (
    SEED
)

state[
    "factorial_batch02_models"
] = (
    6
)

state[
    "factorial_batch02_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch02_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "Batch-02 models before moving to fold0_seed43."
)
...
project_state[
    ...
    "cova3d_factorial_batch02_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-02-PRED-FREEZE."
)
...
def test_batch02_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-02"
    assert state["factorial_runs_completed"] == 12
    assert state["factorial_batches_completed"] == 2
    assert state["factorial_batch02_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch02_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-02 regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: complete COVA-3D factorial batch 01 fits",
)
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-02 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-02 — FINAL REPORT"
)
...
print(
    "Factorial fits completed               : 12 /72"
)

print(
    "Paired fit batches completed           : 2 /12"
)
...
print(
    ...
    "for each model before we move to Batch 02."
)
