# ==========================================================================================
# COVA-3D — BLOCK 10C-FACTORIAL-BATCH-05-PRED-FREEZE
#
# BATCH
# -----
# fold0_seed29
...
BLOCK = (
    "10C-FACTORIAL-BATCH-05-PRED-FREEZE"
)
...
FIT_SUMMARY_PATH = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch05_fit_summary_v1_0.csv"
)
...
PRED_MANIFEST_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch05_prediction_freeze_v1_0.csv"
)
...
AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10c_factorial_batch05_prediction_freeze.json"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch05_prediction_freeze.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10c_factorial_batch05_prediction_freeze.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch05_predictions.sh"
    )
...
heading(
    "COVA-3D 10C — BATCH-05 IMAGE-ONLY PREDICTION FREEZE"
)
...
) != "10B-FACTORIAL-BATCH-05":

    raise RuntimeError(
        "Batch-05 fit is not the latest completed block."
    )


if state.get(
    "factorial_batch05_fit_status"
) != "COMPLETE":

    raise RuntimeError(
        "Batch-05 fit status is not COMPLETE."
    )
...
if len(
    ...
    raise RuntimeError(
        "Expected six Batch-05 fit rows."
    )
...
if set(
    ...
    raise RuntimeError(
        "Unexpected Batch-05 condition set."
    )
...
if len(
    ...
    raise RuntimeError(
        "Execution registry does not contain six Batch-05 runs."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify six Batch-05 execution rows."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify Batch-05 registry row."
    )
...
state[
    ...
    "FACTORIAL_BATCH05_PREDICTIONS_FROZEN_BATCH06_PENDING"
)
...
state[
    "factorial_batch05_prediction_status"
] = (
    "FROZEN"
)

state[
    "factorial_batch05_prediction_manifest"
] = str(
...
state[
    ...
    "Train the six frozen factorial conditions for fold1_seed43. "
...
project_state[
    ...
    "cova3d_factorial_batch06_pending"
)
...
print(
    "✓ Batch-05 execution status            : PREDICTIONS_FROZEN"
)
...
def test_batch05_prediction_manifest():
...
def test_batch05_prediction_audit():
    ...
    assert audit["batch_id"] == "fold1_seed29"
...
def test_batch05_prediction_state():
    ...
    assert state["next_factorial_batch"] == "fold1_seed43"
    assert state["next_block"] == "10B-FACTORIAL-BATCH-06"
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-05 prediction-freeze regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: freeze COVA-3D factorial batch 05 predictions [parallel]",
)
...
heading(
    "COVA-3D 10C — BATCH-05 PREDICTION FREEZE FINAL REPORT"
)
...
print(
    "BATCH-05 IMAGE-ONLY PREDICTIONS"
)
...
print()
print(
    "STATUS                                 : BATCH05_PREDICTIONS_SAFELY_FROZEN"
)
...
print(
    "After I audit this result, Batch 05 will be backed up. Next scientific batch is fold1_seed43."
)
