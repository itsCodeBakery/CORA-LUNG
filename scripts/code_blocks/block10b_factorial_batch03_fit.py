# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-03
#
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch03_epoch_log_v1_0.csv"
)
...
BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch03_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch03_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch03_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch03_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch03_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch03.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-03 — PREFLIGHT"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-02 prediction freeze is not the current completed block."
    )
...
if state.get(
    ...
    raise RuntimeError(
        "Batch-03 is not the next authorized batch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-03 does not contain six runs."
    )
...
state[
    ...
    "FACTORIAL_BATCH03_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch03_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch03_seed"
] = (
    SEED
)

state[
    "factorial_batch03_models"
] = (
    6
)

state[
    "factorial_batch03_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch03_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "Batch-03 models before moving to fold1_seed17."
)
...
project_state[
    ...
    "cova3d_factorial_batch03_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-03-PRED-FREEZE."
)
...
def test_batch03_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-03"
    assert state["factorial_runs_completed"] == 18
    assert state["factorial_batches_completed"] == 3
    assert state["factorial_batch03_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch03_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-03 regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: complete COVA-3D factorial batch 03 fits",
)
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-03 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-03 — FINAL REPORT"
)
...
print(
    "Factorial fits completed               : 18 /72"
)

print(
    "Paired fit batches completed           : 3 /12"
)
...
print(
    ...
    "for each model before we move to Batch 04."
)
