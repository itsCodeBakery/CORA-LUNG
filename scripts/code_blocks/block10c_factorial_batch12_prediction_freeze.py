# ==========================================================================================
# COVA-3D — BLOCK 10C-FACTORIAL-BATCH-12-PRED-FREEZE
#
# BATCH
# -----
# fold3_seed43
...
BLOCK = (
    "10C-FACTORIAL-BATCH-12-PRED-FREEZE"
)

NEXT_BLOCK = (
    "FINAL_FACTORIAL_BRANCH_INTEGRATION_PENDING"
)
...
FIT_SUMMARY_PATH = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch12_fit_summary_v1_0.csv"
)
...
PRED_MANIFEST_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch12_prediction_freeze_v1_0.csv"
)
...
AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10c_factorial_batch12_prediction_freeze.json"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch12_prediction_freeze.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10c_factorial_batch12_prediction_freeze.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch12_predictions.sh"
    )
...
heading(
    "COVA-3D 10C — BATCH-12 IMAGE-ONLY PREDICTION FREEZE"
)
...
) != "10B-FACTORIAL-BATCH-12":

    raise RuntimeError(
        "Batch-12 fit is not the latest completed block."
    )


if state.get(
    "factorial_batch12_fit_status"
) != "COMPLETE":

    raise RuntimeError(
        "Batch-12 fit status is not COMPLETE."
    )
...
if len(
    ...
    raise RuntimeError(
        "Expected six Batch-12 fit rows."
    )
...
if set(
    ...
    raise RuntimeError(
        "Unexpected Batch-12 condition set."
    )
...
if len(
    ...
    raise RuntimeError(
        "Execution registry does not contain six Batch-12 runs."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify six Batch-12 execution rows."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify Batch-12 registry row."
    )
...
state[
    ...
    "FACTORIAL_BATCH12_PREDICTIONS_FROZEN_INTEGRATION_PENDING"
)
...
state[
    "factorial_batch12_prediction_status"
] = (
    "FROZEN"
)

state[
    "factorial_batch12_prediction_manifest"
] = str(
...
state[
    ...
    "Integrate all frozen parallel Batch branches before final dense-outcome evaluation."
)
...
project_state[
    ...
    "cova3d_factorial_branch_integration_pending"
)
...
print(
    "✓ Batch-12 execution status            : PREDICTIONS_FROZEN"
)

print(
    "✓ Prediction-frozen runs               : 42 /72"
)

print(
    "✓ Frozen probability maps              : 168"
)
...
def test_batch12_prediction_manifest():
...
def test_batch12_prediction_audit():
    ...
    assert audit["batch_id"] == "fold3_seed43"
...
def test_batch12_prediction_state():
    ...
    assert state["factorial_runs_completed"] == 42
    assert state["factorial_prediction_runs_frozen"] == 42
    assert state["factorial_prediction_maps_frozen"] == 168

    assert state["final_outer_cv_outcomes_authorized"] is False
    assert state["final_outer_cv_access_in_cova3d"] == 0

    assert state["next_factorial_batch"] == "NONE"
    assert state["next_block"] == "FINAL_FACTORIAL_BRANCH_INTEGRATION_PENDING"
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-12 prediction-freeze regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: freeze COVA-3D factorial batch 12 predictions [parallel]",
)
...
heading(
    "COVA-3D 10C — BATCH-12 PREDICTION FREEZE FINAL REPORT"
)
...
print(
    "BATCH-12 IMAGE-ONLY PREDICTIONS"
)
...
print(
    "Fits represented on branch             : 42 /72"
)

print(
    "Prediction-frozen runs on branch       : 42 /72"
)

print(
    "Frozen probability maps on branch      : 168"
)

print(
    "Fit batches represented on branch      : 7 /12"
)
...
print()
print(
    "STATUS                                 : BATCH12_PREDICTIONS_SAFELY_FROZEN"
)
...
print(
    "After I audit this result, Batch 07 will be backed up. Next scientific batch is NONE."
)
