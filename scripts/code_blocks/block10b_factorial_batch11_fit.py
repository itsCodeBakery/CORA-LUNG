# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-11
#
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch11_epoch_log_v1_0.csv"
)

BATCH_RUN_SUMMARY_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch11_fit_summary_v1_0.csv"
)

BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch11_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch11_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch11_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch11_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch11_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch11.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-11 — PREFLIGHT"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Unexpected frozen main-line state for parallel Batch-11 execution."
    )


if BATCH_ID != "fold3_seed29":

    raise RuntimeError(
        "Parallel Batch-11 authorization mismatch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-11 does not contain six runs."
    )
...
state[
    ...
    "FACTORIAL_BATCH11_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch11_fit_status"
] = (
    "COMPLETE"
)

state[
    "factorial_batch11_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch11_seed"
] = (
    SEED
)

state[
    "factorial_batch11_models"
] = (
    6
)

state[
    "factorial_batch11_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch11_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "Batch-11 models before moving to fold3_seed43."
)
...
project_state[
    ...
    "cova3d_factorial_batch11_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-11-PRED-FREEZE."
)
...
def test_batch11_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-11"
    assert state["factorial_runs_completed"] == 36
    assert state["factorial_batches_completed"] == 6
    assert state["factorial_batch11_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch11_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-11 regression tests failed."
    )
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-11 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-11 — FINAL REPORT"
)
...
print(
    "Factorial fits represented on branch   : 36 /72"
)

print(
    "Fit batches represented on branch      : 6 /12"
)
...
print(
    ...
    "for each model before this parallel Batch-11 run is backed up."
)
