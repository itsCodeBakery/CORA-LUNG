# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-08
#
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch08_epoch_log_v1_0.csv"
)
...
BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch08_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch08_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch08_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch08_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch08_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch08.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-08 — PREFLIGHT"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Unexpected frozen main-line state for parallel Batch-08 execution."
    )


if BATCH_ID != "fold2_seed29":

    raise RuntimeError(
        "Parallel Batch-08 authorization mismatch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-08 does not contain six runs."
    )
...
state[
    ...
    "FACTORIAL_BATCH08_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch08_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch08_seed"
] = (
    SEED
)

state[
    "factorial_batch08_models"
] = (
    6
)

state[
    "factorial_batch08_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch08_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "Batch-08 models before moving to fold2_seed43."
)
...
project_state[
    ...
    "cova3d_factorial_batch08_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-08-PRED-FREEZE."
)
...
def test_batch08_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-08"
    assert state["factorial_runs_completed"] == 24
    assert state["factorial_batches_completed"] == 4
    assert state["factorial_batch08_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch08_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-08 regression tests failed."
    )
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-08 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-08 — FINAL REPORT"
)
...
print(
    ...
    "for each model before this parallel Batch-08 run is backed up."
)
