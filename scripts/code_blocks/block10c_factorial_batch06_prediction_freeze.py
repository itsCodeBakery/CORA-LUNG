# ==========================================================================================
# COVA-3D — BLOCK 10C-FACTORIAL-BATCH-06-PRED-FREEZE
#
# BATCH
# -----
# fold0_seed29
...
BLOCK = (
    "10C-FACTORIAL-BATCH-06-PRED-FREEZE"
)

NEXT_BLOCK = (
    "10B-FACTORIAL-BATCH-07"
)
...
FIT_SUMMARY_PATH = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch06_fit_summary_v1_0.csv"
)
...
PRED_MANIFEST_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch06_prediction_freeze_v1_0.csv"
)
...
AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10c_factorial_batch06_prediction_freeze.json"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch06_prediction_freeze.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10c_factorial_batch06_prediction_freeze.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch06_predictions.sh"
    )
...
heading(
    "COVA-3D 10C — BATCH-06 IMAGE-ONLY PREDICTION FREEZE"
)
...
) != "10B-FACTORIAL-BATCH-06":

    raise RuntimeError(
        "Batch-06 fit is not the latest completed block."
    )


if state.get(
    "factorial_batch06_fit_status"
) != "COMPLETE":

    raise RuntimeError(
        "Batch-06 fit status is not COMPLETE."
    )
...
if len(
    ...
    raise RuntimeError(
        "Expected six Batch-06 fit rows."
    )
...
if set(
    ...
    raise RuntimeError(
        "Unexpected Batch-06 condition set."
    )
...
if len(
    ...
    raise RuntimeError(
        "Execution registry does not contain six Batch-06 runs."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify six Batch-06 execution rows."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify Batch-06 registry row."
    )
...
state[
    ...
    "FACTORIAL_BATCH06_PREDICTIONS_FROZEN_BATCH07_PENDING"
)
...
state[
    "factorial_batch06_prediction_status"
] = (
    "FROZEN"
)

state[
    "factorial_batch06_prediction_manifest"
] = str(
...
state[
    ...
    "Train the six frozen factorial conditions for fold2_seed17. "
...
project_state[
    ...
    "cova3d_factorial_batch07_pending"
)
...
print(
    "✓ Batch-06 execution status            : PREDICTIONS_FROZEN"
)

print(
    "✓ Prediction-frozen runs               : 24 /72"
)

print(
    "✓ Frozen probability maps              : 96"
)
...
def test_batch06_prediction_manifest():
...
def test_batch06_prediction_audit():
    ...
    assert audit["batch_id"] == "fold1_seed43"
...
def test_batch06_prediction_state():
    ...
    assert state["factorial_prediction_runs_frozen"] == 24
    assert state["factorial_prediction_maps_frozen"] == 96

    assert state["final_outer_cv_outcomes_authorized"] is False
    assert state["final_outer_cv_access_in_cova3d"] == 0

    assert state["next_factorial_batch"] == "fold2_seed17"
    assert state["next_block"] == "10B-FACTORIAL-BATCH-07"
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-06 prediction-freeze regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: freeze COVA-3D factorial batch 06 predictions [parallel]",
)
...
heading(
    "COVA-3D 10C — BATCH-06 PREDICTION FREEZE FINAL REPORT"
)
...
print(
    "BATCH-06 IMAGE-ONLY PREDICTIONS"
)
...
print(
    "Fits represented on branch             : 24 /72"
)

print(
    "Prediction-frozen runs on branch       : 24 /72"
)

print(
    "Frozen probability maps on branch      : 96"
)

print(
    "Fit batches represented on branch      : 4 /12"
)
...
print()
print(
    "STATUS                                 : BATCH06_PREDICTIONS_SAFELY_FROZEN"
)
...
print(
    "After I audit this result, Batch 06 will be backed up. Next scientific batch is fold2_seed17."
)
