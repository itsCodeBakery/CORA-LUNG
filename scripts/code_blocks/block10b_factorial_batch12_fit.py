# ==========================================================================================
# COVA-3D — BLOCK 10B-FACTORIAL-BATCH-12
#
...
BATCH_EPOCH_LOG_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch12_epoch_log_v1_0.csv"
)
...
BATCH_AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10b_factorial_batch12_fit.json"
)

FIG_PNG = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch12_training_curves.png"
)

FIG_PDF = (
    REPO
    / "figures/audit/"
    "fig_cova3d_factorial_batch12_training_curves.pdf"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch12_fit.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10b_factorial_batch12_fit.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch12.sh"
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-12 — PREFLIGHT"
)
...
if state.get(
    ...
    raise RuntimeError(
        "Unexpected frozen main-line state for parallel Batch-12 execution."
    )


if BATCH_ID != "fold3_seed43":

    raise RuntimeError(
        "Parallel Batch-12 authorization mismatch."
    )
...
if len(
    ...
    raise RuntimeError(
        "Batch-12 does not contain six runs."
    )
...
state[
    ...
    "FACTORIAL_BATCH12_FIT_COMPLETE_PREDICTION_FREEZE_PENDING"
)
...
state[
    "factorial_batch12_outer_fold"
] = (
    OUTER_FOLD
)

state[
    "factorial_batch12_seed"
] = (
    SEED
)

state[
    "factorial_batch12_models"
] = (
    6
)

state[
    "factorial_batch12_shared_initialization_sha256"
] = (
    initialization_sha
)

state[
    "factorial_batch12_paired_plan_sha256"
] = (
    paired_plan_sha
)
...
state[
    ...
    "Batch-12 models. This is the final registered training batch; branch integration follows after prediction freeze."
)
...
project_state[
    ...
    "cova3d_factorial_batch12_prediction_freeze_pending"
)
...
project_state[
    ...
    "Run 10C-FACTORIAL-BATCH-12-PRED-FREEZE."
)
...
def test_batch12_state():
    ...
    assert state["last_completed_block"] == "10B-FACTORIAL-BATCH-12"
    assert state["factorial_runs_completed"] == 42
    assert state["factorial_batches_completed"] == 7
    assert state["factorial_batch12_fit_status"] == "COMPLETE"
...
tests = sh(
    [
        ...
        "tests/test_cova3d_factorial_batch12_fit.py",
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-12 regression tests failed."
    )
...
if final_status:
    ...
    raise RuntimeError(
        "Repository not clean after Batch-12 commit:\n"
        + final_status
    )
...
heading(
    "COVA-3D 10B-FACTORIAL-BATCH-12 — FINAL REPORT"
)
...
print(
    "Factorial fits represented on branch   : 42 /72"
)
...
print(
    ...
    "for each model before this parallel Batch-12 run is backed up."
)
