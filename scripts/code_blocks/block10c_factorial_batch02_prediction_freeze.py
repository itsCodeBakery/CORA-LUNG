# ==========================================================================================
# COVA-3D — BLOCK 10C-FACTORIAL-BATCH-02-PRED-FREEZE
#
# BATCH
# -----
# fold0_seed29
...
FIT_SUMMARY_PATH = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch02_fit_summary_v1_0.csv"
)
...
PRED_MANIFEST_REPO = (
    REPO
    / "data/manifests/"
    "cova3d_factorial_batch02_prediction_freeze_v1_0.csv"
)
...
AUDIT_PATH = (
    REPO
    / "experiments/audits/"
    "block10c_factorial_batch02_prediction_freeze.json"
)

TEST_PATH = (
    REPO
    / "tests/"
    "test_cova3d_factorial_batch02_prediction_freeze.py"
)

SOURCE_PATH = (
    REPO
    / "scripts/code_blocks/"
    "block10c_factorial_batch02_prediction_freeze.py"
)
...
def make_git_auth():
    ...
    askpass = Path(
        "/tmp/cova3d_git_askpass_batch02_predictions.sh"
    )
...
heading(
    "COVA-3D 10C — BATCH-02 IMAGE-ONLY PREDICTION FREEZE"
)
...
) != "10B-FACTORIAL-BATCH-02":

    raise RuntimeError(
        "Batch-02 fit is not the latest completed block."
    )


if state.get(
    "factorial_batch02_fit_status"
) != "COMPLETE":

    raise RuntimeError(
        "Batch-02 fit status is not COMPLETE."
    )
...
if len(
    ...
    raise RuntimeError(
        "Expected six Batch-02 fit rows."
    )
...
if set(
    ...
    raise RuntimeError(
        "Unexpected Batch-02 condition set."
    )
...
if len(
    ...
    raise RuntimeError(
        "Execution registry does not contain six Batch-02 runs."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify six Batch-02 execution rows."
    )
...
if int(
    ...
    raise RuntimeError(
        "Could not identify Batch-02 registry row."
    )
...
state[
    ...
    "FACTORIAL_BATCH02_PREDICTIONS_FROZEN_BATCH03_PENDING"
)
...
state[
    "factorial_batch02_prediction_status"
] = (
    "FROZEN"
)

state[
    "factorial_batch02_prediction_manifest"
] = str(
...
state[
    ...
    "Train the six frozen factorial conditions for fold0_seed43. "
...
project_state[
    ...
    "cova3d_factorial_batch03_pending"
)
...
print(
    "✓ Batch-02 execution status            : PREDICTIONS_FROZEN"
)

print(
    "✓ Prediction-frozen runs               : 6 /72"
)

print(
    "✓ Frozen probability maps              : 24"
)
...
def test_batch02_prediction_manifest():
...
def test_batch02_prediction_audit():
    ...
    assert audit["batch_id"] == "fold0_seed29"
...
def test_batch02_prediction_state():
    ...
    assert state["factorial_runs_completed"] == 12
    assert state["factorial_prediction_runs_frozen"] == 12
    assert state["factorial_prediction_maps_frozen"] == 48

    assert state["final_outer_cv_outcomes_authorized"] is False
    assert state["final_outer_cv_access_in_cova3d"] == 0

    assert state["next_factorial_batch"] == "fold0_seed43"
    assert state["next_block"] == "10B-FACTORIAL-BATCH-03"
...
if tests.returncode != 0:
    ...
    raise RuntimeError(
        "Batch-02 prediction-freeze regression tests failed."
    )
...
git(
    "commit",
    "-m",
    "experiment: freeze COVA-3D factorial batch 02 predictions",
)
...
heading(
    "COVA-3D 10C — BATCH-02 PREDICTION FREEZE FINAL REPORT"
)
...
print(
    "BATCH-02 IMAGE-ONLY PREDICTIONS"
)
...
print(
    "Fits completed                         : 12 /72"
)

print(
    "Prediction-frozen runs                 : 12 /72"
)

print(
    "Frozen probability maps                : 48"
)

print(
    "Training batches completed             : 2 /12"
)
...
print()
print(
    "STATUS                                 : BATCH_01_PREDICTIONS_SAFELY_FROZEN"
)
...
print(
    "After I audit this result, we move to Batch 03: fold0_seed43."
)
